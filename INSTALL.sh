...
if [ "$DRY_RUN" = "1" ]; then
  ...
  if [ -f "$PI_BUNDLE" ]; then
    # Stage via mktemp: fixed names in world-writable /tmp collide with
    # leftovers from other users and can be pre-created/hijacked.
    WRAPPER_TMP="$(mktemp)"
...
exec "$NODE" "$PI_BUNDLE" "$@"
WRAPPER
    sed -i "s|__VOLTA_HOME__|$HOME/.volta|g; s|__USER__|$USER|g" "$WRAPPER_TMP"
    # Best-effort like apt_install: a sudo failure (no TTY / no NOPASSWD) must
    # not abort the whole installer under set -e.
    if sudo install -m 0755 "$WRAPPER_TMP" /usr/local/bin/pi; then
  ...
  # vim/nvim: same sudo behavior as pi (snapshot invoking user's config into
  # /root, then run root's own binary). One generator; only the snapshot body
  # differs. Distros binaries only (avoids clobbering e.g. brew on mac).
  install_sudo_snapshot_wrapper() {
    local name="$1" exec_bin="$2" snapshot="$3" tmp
    tmp="$(mktemp)"
    cat >"$tmp" <<WRAPPER
#!/usr/bin/env bash
# Generated by dotfiles INSTALL.sh. Via sudo: snapshot the invoking user's
# config into root's home first, then run root's own binary. Root writes ONLY
# root-owned files under /root — the user's config is never written by root,
# so it can never be polluted or locked out.
UHOME=""
if [ "\$(id -u)" = "0" ] && [ -n "\${SUDO_USER:-}" ] && [ "\$SUDO_USER" != "root" ]; then
  UHOME="\$(getent passwd "\$SUDO_USER" | cut -d: -f6)"
fi
...
exec $exec_bin "\$@"
WRAPPER
    if sudo install -m 0755 "$tmp" "/usr/local/bin/$name"; then
  ...
  if [ "$(command -v nvim 2>/dev/null)" = "/usr/bin/nvim" ]; then
    install_sudo_snapshot_wrapper nvim /usr/bin/nvim '  if [ -d "$UHOME/.config/nvim/" ]; then
    if command -v rsync >/dev/null 2>&1; then
...
if infocmp alacritty >/dev/null 2>&1; then
  ...
  apt_install curl || true
  TERMINFO_SRC="$(mktemp)"
  curl -fsSL "$ALACRITTY_REPO/raw/master/extra/alacritty.info" -o "$TERMINFO_SRC"
  tic -xe alacritty,alacritty-direct "$TERMINFO_SRC"
  rm -f "$TERMINFO_SRC"
